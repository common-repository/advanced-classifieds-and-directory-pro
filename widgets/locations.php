...
class ACADP_Widget_Locations extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $has_location ) {		
			$query_args = array(
				...
				'orderby'        => isset( $instance['orderby'] ) ? $instance['orderby'] : $locations_settings['orderby'], 
    			'order'          => isset( $instance['order'] ) ? $instance['order'] : $locations_settings['order'], 
			...
			if ( isset( $misc_settings['force_bootstrap'] ) && ! empty( $misc_settings['force_bootstrap'] ) ) {
				...
				if ( ! empty( $locations ) ) {	
					...
					echo '<span>' . __( 'No locations found', 'advanced-classifieds-and-directory-pro' ) . '</span>';
				}
			} else {
				$attributes = $query_args;

				$template = $attributes['template'];
				$allowed_templates = array( 'dropdown', 'list' );
				
				if ( ! in_array( $template, $allowed_templates ) ) {
					$template = 'list';
				}

				include apply_filters( 'acadp_load_template', ACADP_PLUGIN_DIR . "widgets/templates/locations/layout-{$template}.php" );
			}	
	...
	public function form( $instance ) {
		...
		include ACADP_PLUGIN_DIR . 'widgets/forms/locations.php';
	}
