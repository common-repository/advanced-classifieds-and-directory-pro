	...
	public function run_shortcode_categories( $atts ) {	
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			if ( 'image_grid' == $attributes['view'] ) {
				$deprecated = array( 'template' => 'categories/acadp-public-categories-image-grid-display.php' );
				include apply_filters( 'acadp_load_template', ACADP_PLUGIN_DIR . 'public/templates/categories/layout-grid.php', array( 'deprecated' => $deprecated ) );
			} else {
				$deprecated = array( 'template' => 'categories/acadp-public-categories-text-list-display.php' );
				include apply_filters( 'acadp_load_template', ACADP_PLUGIN_DIR . 'public/templates/categories/layout-list.php', array( 'deprecated' => $deprecated ) );
			}
			return ob_get_clean();		
		}

		return '<span>' . __( 'No categories found', 'advanced-classifieds-and-directory-pro' ) . '</span>';		
	}
