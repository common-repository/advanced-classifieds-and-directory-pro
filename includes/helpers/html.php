...
function acadp_get_terms_dropdown_html( $params = array() ) {
	...
	if ( isset( $general_settings['select_box'] ) && 'default' == $general_settings['select_box'] ) {
		$select_box = 'default';
	}
	...
	if ( 'custom' == $select_box ) {
		$html = acadp_get_terms_searchable_dropdown_html( $params );		
	} else {
		$html = acadp_get_terms_standard_dropdown_html( $params );
	}
...
/**
 * Standard HTML dropdown for categories and locations.
 *
 * @since  3.1.0
 * @param  array  $params Array of params input.
 * @return string         Dropdown HTML string.
 */
function acadp_get_terms_standard_dropdown_html( $params = array() ) {
	// Vars
	$defaults = array(
	...
	// Classes
	$classes = array( 'form-control', 'acadp-form-control', 'acadp-form-select' );

	if ( $attributes['required'] ) {
		$classes[] = 'acadp-form-validate';
	}
	...
	if ( is_array( $attributes['selected'] ) && ! empty( $attributes['selected'] ) ) {
		$selected = $attributes['selected'][0];
	}

	// Populate
	$args = array(
		'show_option_none'  => sanitize_text_field( $attributes['placeholder'] ),
		'option_none_value' => '',
		'taxonomy'          => sanitize_text_field( $attributes['taxonomy'] ),
		'child_of'          => (int) $attributes['parent'],
...
/**
 * Searchable dropdown for categories and locations.
 *
 * @since  3.1.0
 * @param  array  $params Array of params input.
 * @return string         Dropdown HTML string.
 */
function acadp_get_terms_searchable_dropdown_html( $params = array() ) {
	// Vars	
	$defaults = array(
		'placeholder'     => '— ' . esc_html__( 'Select Category', 'advanced-classifieds-and-directory-pro' ) . ' —',
		'taxonomy'        => 'acadp_categories',
	...
	$field_classes = array( 'acadp-term-input', 'acadp-form-control', 'acadp-form-' . $field_type );
	...
	if ( 1 === $attributes['level'] ) {
		$html .= sprintf( 
			'<acadp-dropdown-terms id="%s" class="%s" data-type="%s" data-name="%s" data-taxonomy="%s" data-required="%s" data-close_on_select="%s">', 
			...
			esc_attr( $attributes['close_on_select'] )
		);

		// Select
		$html .= '<div class="acadp-dropdown-select">';
		...
		$html .= '<button type="button" class="acadp-button-clear" hidden>
		...
		// Dropdown 
		$html .= '<div class="acadp-dropdown" hidden>';
		$html .= '<div class="acadp-dropdown-content">';

		// Dropdown: Search
		$html .= sprintf( '<div class="acadp-dropdown-header"%s>', ( $attributes['search_disabled'] ? ' hidden' : '' ) );
		...
		// Dropdown: Body
		$html .= '<div class="acadp-dropdown-body">';
	}
	...
	if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) { 
		...
		$html .= sprintf( 
			'<div class="acadp-terms acadp-terms-%s" data-level="%d">', 
			( 1 === $attributes['level'] ? 'parent' : 'children' ),
			$attributes['level'] 
		);

		foreach ( $terms as $term ) { 
			$can_load_children = false;
			if ( in_array( $term->term_id, $attributes['ancestors'] ) || ( $attributes['parent_disabled'] && 1 === $attributes['level'] ) ) {
				$can_load_children = true;
			}

			$html .= sprintf( '<div class="%s">', ( $can_load_children ? 'acadp-term acadp-terms-children-populated' : 'acadp-term' ) );

			// Label
			if ( 0 === $term->parent && $attributes['parent_disabled'] ) {
				$html .= sprintf( '<label class="acadp-term-label" style="padding-left: %dpx;" disabled>', $attributes['level'] * 16 );
				$html .= sprintf( '<span class="acadp-term-name">%s</span>', esc_html( $term->name ) );
				$html .= '</label>';
			} else {
				$html .= sprintf( '<label class="acadp-term-label" style="padding-left: %dpx;">', $attributes['level'] * 16 );				
				$html .= sprintf( '<span class="acadp-term-name">%s</span>', esc_html( $term->name ) );
	...
	if ( 1 === $attributes['level'] ) {
		// Dropdown: Status
		$html .= sprintf(
			'<div class="acadp-dropdown-status"%s>%s</div>',
			( ! $terms_found ? '' : ' hidden' ),
		...
		// Dropdown: Footer
		$html .= '<div class="acadp-dropdown-footer">';
		$html .= sprintf( '<a href="javascript:void(0)" class="acadp-dropdown-close">%s</a>', esc_attr__( 'Close', 'advanced-classifieds-and-directory-pro' ) );
		$html .= '</div>';
